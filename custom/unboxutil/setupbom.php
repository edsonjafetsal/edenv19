...
$arrayofparameters = array(
    'BOMFIFO'=>array('css'=>'minwidth500', 'enabled'=>1),
    'BOMLIFO'=>array('css'=>'minwidth500', 'enabled'=>1),
    'BOMFEFO'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
    ...
    {
        $file = dol_buildpath($reldir."core/modules/unboxutil/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
        if (file_exists($file))
        {
            $filefound = 1;
...
elseif ($action == 'setmod') {
    ...
    if (!empty($tmpobjectkey)) {
        $constforval = 'unboxutil_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
    ...
    if ($ret > 0) {
        $tmpobjectkey = GETPOST('object');
        if (!empty($tmpobjectkey)) {
            $constforval = 'unboxutil_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print dol_get_fiche_head($head, 'bomfifo', $langs->trans("Fifo Setup"), -1, 'bom');

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($title, $linkback, 'object_unboxutil@unboxutil');

// Configuration header
//$head = unboxutilAdminPrepareHead();
//print dol_get_fiche_head($head, 'settings', '', -1, "unboxutil@unboxutil");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("Utils Bom Setup").'</span><br><br>';
...
if ($action == 'edit')
{
    ...
    {
        print '<table class="noborder centpercent">';
        print '<tr class="liste_titre"><td class="titlefield">'.$langs->trans("Parameter").'</td><td></td><td>'.$langs->trans("Value").'</td></tr>';
        ...
        foreach ($arrayofparameters as $key => $val)
        {
           ...
           // print '</td><td>'.ajax_constantonoff($conf->global->$key).'</td></tr>';
            print '</td><td>';
            print '</td><td>'.ajax_constantonoff($key).'</td></tr>';
        /*    if (empty($conf->global->$key))
            {
                print '<a href="'.$_SERVER['PHP_SELF'].'?action=set_'.$key.'&amp;token='.newToken().'">'.img_picto($langs->trans("Disabled"), 'off').'</a>';
            } else {
                print '<a href="'.$_SERVER['PHP_SELF'].'?action=del_'.$key. '&amp;token='.newToken().'">'.img_picto($langs->trans("Enabled"), 'on').'</a>';
            }*/
            print '</td></tr>';
        }
        print '</table>';
                    ...
                    while (($file = readdir($handle)) !== false)
                    {
                        if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
                        {
                            ...
                            if ($module->isEnabled())
                            {
                                ...
                                $constforvar = 'unboxutil_'.strtoupper($myTmpObjectKey).'_ADDON';
