...
//                if ((count($prodsfather) > 0) && ($object->desdepadre == 0)) {
				if ((count($prodsfather) > 0)) {
					foreach ($prodsfather as $value) {
						...
						$sumar = $resto ? 1 : 0;
						if($object->qty > 0) {
							$qtypadrearbajar = number_format(abs(floor((float)$object->qty / (float)$value["qty"])),0);
						}elseif ($object->qty < 0){
							$qtypadrearbajar = number_format(abs(floor((float)$object->qty / (float)$value["qty"])),0)*-1;
						}
						if ($qtypadrearbajar) {
            ...
            case 'LINEBILL_SUPPLIER_CREATE':
                $sql ="UPDATE "  . MAIN_DB_PREFIX . "facture_fourn_det AS fd SET fd.fk_code_ventilation  = ( SELECT fex.invoicetype
                FROM llx_facture_fourn_extrafields AS fex
                WHERE fex.fk_object = ". $object->fk_facture_fourn. ")  WHERE fd.rowid = ". $object->id;
            ...
            case 'SHIPPING_VALIDATE':
				...
				$sql2 = " UPDATE ". MAIN_DB_PREFIX."expedition SET date_expidition = CURDATE() ";
