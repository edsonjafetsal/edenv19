...
//			if (empty($batch))
            ...
            //TODO RAZMI  aqui buscar el mayor max id o el mayor eatby
            //PRIMER FIFO sino hay entonces el mas viejo por rowid mas viejo porque por lotes como es texto no se puede controlar

            $sql = "SELECT min(lpb.eatby) AS caducidad,batch from " . MAIN_DB_PREFIX . "product_lot lpb WHERE lpb.rowid=" . $fk_product . " limit 1";
            ...
            if ($resql) {
                ...
                if ($num > 0) {
                    while ($i < $num) {
                        $productobatch = $this->db->fetch_object($resql);
                        if (!$batch) $batch = $productobatch->batch;
                        $i += 1;
                    }
                } else {
                    $sql = "SELECT min(lpb.rowid) AS caducada,rowid as id from " . MAIN_DB_PREFIX . "product_lot lpb WHERE lpb.rowid=" . $fk_product . " limit 1";
                    $num = $this->db->num_rows($resql);
                    $i = 0;
                    if ($num > 0) {
                        while ($i < $num) {
                            $productobatch = $this->db->fetch_object($resql);
                            $fk_product = $productobatch->id;
                            $i += 1;
                        }
            ...
            if ($batch) {
                ...
                $sql .= " WHERE pb.fk_product = " . $fk_product;
        ...
        if (!$error && !empty($conf->global->PRODUIT_SOUSPRODUITS) && empty($conf->global->INDEPENDANT_SUBPRODUCT_STOCK) && empty($disablestockchangeforsubproduct)) {
            if (!$isfather) {//FIXME Razmi pasar Lotes o batch a esta funcion
                $error = $this->_createSubProduct($user, $fk_product, $entrepot_id, $qty, $type, 0, $label, $inventorycode,$batch); // we use 0 as price, because AWP must not change for subproduct
                $this->desdepadre = 1;
            }
     ...
     */
    private function _createSubProduct($user, $idProduct, $entrepot_id, $qty, $type, $price = 0, $label = '', $inventorycode = '',$batch='')
    {
        global $langs;
        ...
        foreach ($pids as $key => $value) {
            if (!$error) {
                ...
                $result = $tmpmove->_create($user, $pids[$key], $entrepot_id, ($qty * $pqtys[$key]), $type, 0, $label, $inventorycode,'','','',$batch); // This will also call _createSubProduct making this recursive
                if ($result < 0) {
...
//TODO Razmi verificar si tiene hijos sino desdepadre=1
        return $error;
    }
