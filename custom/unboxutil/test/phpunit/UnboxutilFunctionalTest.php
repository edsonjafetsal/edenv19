...
/**
 * \file    test/phpunit/UnboxutilFunctionalTest.php
 * \ingroup unboxutil
 ...
 *
 * @package Testunboxutil
 */
class UnboxutilFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modUnboxutil')->click();
		} else {
			// Disable the module
			$this->byHref('modUnboxutil')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/unboxutil/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('unboxutil/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/unboxutil/admin/about.php');
		$this->authenticate();
		return $this->assertContains('unboxutil/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('unboxutilwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modUnboxutil_UnboxutilTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modUnboxutil_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
