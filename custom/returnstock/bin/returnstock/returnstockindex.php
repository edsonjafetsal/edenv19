...
/**
 *	\file       returnstock/returnstockindex.php
 *	\ingroup    returnstock
 *	\brief      Home page of returnstock top menu
 */
...
$langs->loadLangs(array("returnstock@returnstock"));

$action = GETPOST('action', 'aZ09');


// Security check
//if (! $user->rights->returnstock->myobject->read) accessforbidden();
...
llxHeader("", $langs->trans("ReturnStockArea"));

print load_fiche_titre($langs->trans("ReturnStockArea"), '', 'returnstock.png@returnstock');
...
// Draft MyObject
if (! empty($conf->returnstock->enabled) && $user->rights->returnstock->read)
{
	$langs->load("orders");
...
if (! empty($conf->returnstock->enabled) && $user->rights->returnstock->read)
{
	...
	$sql.= " FROM ".MAIN_DB_PREFIX."returnstock_myobject as s";
	//if (! $user->rights->societe->client->voir && ! $socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
			...
			if ($resql12) {
				...
				while ($i < $num) {
					...
					if ($resql11)
					{
						...
						setEventMessages('Updated Stock', '', 'mesgs');
