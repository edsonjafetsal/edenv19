...
/*
 * Copyright    2018-2021   ProgSI    <contact@progsi.ma>
 *
 */

// -- bugfix V13 : le html dans les traductions n'est plus permis
$('a.vmenu').each(function(index){
	if($(this).attr('href').indexOf('kanview_config.php') > 0){
		var text = $(this).text();
		$(this).html('<em style="color: blue;">' + text + '</em>');
	}
});;

var kanview = {};			// espace de nom pour le module
...
//
kanview.getCookie = function (cname){
	var name = cname + "=";
...
//
kanview.isKeyValidForDate = function (key){
...
//
kanview.toggleSelectRow = function (element){
...
kanview.getUrlParams = function (url){
	var urlParams = '';
	var sortParams = '';
	var params = null;
	var one_param = null;
...
$(function (){
	var action_principal = kanview.getCookie('action_principal');		// action en cours de la page principale
	if (action_principal === ''){
...
function compareVersions (version1, version2){
	...
	var parts1 = version1.split('.').map(function (i){
		return parseInt(i);
	});
	var parts2 = version2.split('.').map(function (i){
		return parseInt(i);
	});
...
// ---------------------------------------------------------------------------------------------------
//
// vérifie si un élément jQuery est dans le champ visible de l'écran (après un scroll par exemple)
// (ceci est différent de visible/hidden)
function isElementVisibleOnView (elementToBeChecked){
