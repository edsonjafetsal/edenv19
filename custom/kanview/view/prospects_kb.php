...
$isDateStartSet = (isset($_POST['search_dd_datec_day']) || isset($_GET['search_dd_datec_day']));
$isDateEndSet   = (isset($_POST['search_df_datec_day']) || isset($_GET['search_df_datec_day']));

$isDateStartNull = $isDateStartSet && empty(GETPOST('search_dd_datec_day', 'alpha'));
$isDateEndNull   = $isDateEndSet && empty(GETPOST('search_df_datec_day', 'alpha'));
...
$search_fk_commercial           = GETPOST('search_fk_commercial', 'int');
...
//
// -------------------- >>> action cardDrop 
//                          après Drag&Drop d'une tuile ==> mise à jour du "Status" de l'objet
//
if ($action == 'cardDrop') {
...
//                           on remplit la liste des départements du pays
//
...
if ($search_dd_datec_mysql != '' && $search_df_datec_mysql != '') {
  ...
  if (!$isDateStartNull && !$isDateEndNull) {
    $WHERE .= " AND (t.datec BETWEEN '" . $db->idate($search_dd_datec_mysql) . "' AND '" . $db->idate($search_df_datec_mysql) . "')";
  }
  elseif (!$isDateStartNull && $isDateEndNull) {
    $WHERE .= " AND (t.datec >= '" . $db->idate($search_dd_datec_mysql) . "')";
  }
  elseif ($isDateStartNull && !$isDateEndNull) {
    $WHERE .= " AND (t.datec <= '" . $db->idate($search_df_datec_mysql) . "')";
  }
}
...
if (intval($search_fk_commercial) > 0)
  $HAVING .= " AND CONCAT(',', GROUP_CONCAT(DISTINCT sc.fk_user), ',') LIKE '%," . intval($search_fk_commercial) . ",%'";
...
// 
// -------------------------- >>> nbre d'éléments dans une colonnes. 
// 
// on utilise :
...
if ($search_fk_commercial != '')
  $params .= '&search_fk_commercial=' . urlencode($search_fk_commercial);
...
//
// date start
$value = (empty($search_dd_datec) || ($isDateStartSet && $isDateStartNull) ? - 1 : $search_dd_datec);
...
// date end
$value = (empty($search_df_datec) || ($isDateEndSet && $isDateEndNull) ? - 1 : $search_df_datec);
...
$values         = 'TYPE_BOTH,TYPE_PROSPECT_ONLY,TYPE_PROSPECT_CLIENT_ONLY';
$keys           = '5,2,3';
...
// --------------------------------------------- commercial
//
echo '<tr id="tr-search_fk_commercial" class="tr-external-filter">';
echo '<td class="td-card-label">' . $langs->trans('ReqKbMainProspects_Fieldfk_commercial') . '</td>';
echo '<td>' . '' . '</td>';
echo '<td id="fk_commercial_td_filtre" class="liste_filtre" align="" valign="">';
echo '<div class="div-external-filter">';
// ------------- filtre --req_kb_main_prospects-- fk_commercial (fk_user)
$SQL               = "
					SELECT DISTINCT u.rowid, u.firstname, u.lastname
					FROM " . MAIN_DB_PREFIX . "societe_commerciaux sc 
          LEFT JOIN " . MAIN_DB_PREFIX . "user u ON sc.fk_user = u.rowid
					ORDER BY u.firstname, u.lastname
					";
$sql_fk_commercial = $db->query($SQL);
$options           = array();
$optionSelected    = '';
$defaultValue      = '';
$blanckOption      = 1;
if ($sql_fk_commercial) {
  while ($obj_fk_commercial = $db->fetch_object($sql_fk_commercial)) {
    if ($obj_fk_commercial->rowid == $search_fk_commercial) {
      $optionSelected = $obj_fk_commercial->rowid;
    }
    $options[$obj_fk_commercial->rowid] = implode(' ', array($obj_fk_commercial->firstname, $obj_fk_commercial->lastname));
  }
  $db->free($sql_fk_commercial);
}
else {
  dol_syslog(__FILE__ . ' - ' . __LINE__ . ' - ' . $db->lasterror(), LOG_ERR);
}
echo '<select  id="fk_commercial_input_filtre" class="flat" name="search_fk_commercial" title="">';
if (empty($optionSelected) && !$blanckOption) {
  $optionSelected = $defaultValue;
}
if ($blanckOption) {
  echo '<option value="0" ' . (empty($search_fk_commercial) ? 'selected' : '') . '></option>';
}
foreach ($options as $key => $value) {
  echo '<option value="' . $key . '" ' . ($key == $optionSelected ? 'selected' : '') . '>' . (empty($value) ? '' : $value) . '</option>';
}
echo '</select>';
echo '</div>';
echo '</td>';
echo '</tr>';
// echo ajax_combobox('fk_commercial_input_filtre');
/// ---
