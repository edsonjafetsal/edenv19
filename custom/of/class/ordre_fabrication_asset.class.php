 ...
 * */
	...
	function reStockAsset(&$PDOdb, $qty_to_re_stock)
	{
        ...
        foreach($TAsset as $asset)
        {
			...
			$labelMvt = 'Production order deletion';
			if($this->type == 'TO_MAKE') $labelMvt = 'Production order deletion';

         	$asset->save($PDOdb,$user
	            ,$labelMvt.' n°'.$OF->numero.' - Equipment : '.$asset->serial_number
...
//                $TAsset->contenancereel_value = $qty_to_make_asset;
				...
				if(!$fk_entrepot) exit('ASSET_USE_DEFAULT_WAREHOUSE not defined in the module configuration.. ERR.L.2244');
	...
	function getWorkstationsPDF(&$db) //TODO AA what the fuck is this function name ?!
	{
    ...
    //TODO redo, it's coded by mother michèle and I think her cat blew her brains out
	...
	function getLibelleEntrepot(&$PDOdb, $withStock=true)
	{
		...
		if (!$this->fk_entrepot) return 'No warehouse selected';
		...
		if ($res) return $res;
		else return 'No warehouse selected';
	}
	...
	function stockQtyToMakeAsset(&$PDOdb, &$of, $fromCloseOf=0)
	{
		...
		//ODO if no equipment defined, no stock movement! to correct
...
class TAssetWorkstationOF extends TObjetStd{
	...
	public function calculTaskDates($PDOdb, $OF, $ws, $projectTask, $date_start_search = null, $TExcludeTaskId = array())
    {
        ...
        $res_date_start = $res_date_end = null; // TODO it is necessary to take into account here, an offset for the start date if there is a family link between the tasks
        ...
        if ($date_start_search === null)
        {
            ...
            if (empty($date_start_search))
            {
                ...
                else
                {
                    ...
                    $date_start_search = strtotime(' +'.$delta.' days', $OF->date_besoin); // Completely inconsistent TODO based on the date of the requirement
                }
            }
            else
            {
        ...
        if (!empty($conf->workstationatm->enabled))
        {
            // TODO deserves a little otpimization by passing the timestamp of the end date of the parent task as a parameter rather than fetcher it here
...
//            }
            ...
            // TODO this loop as well as the init of the variables just above must be in a fully-fledged method because it must be called during the update of the tasks
            ...
            // TODO see if we don't put 23:59:59 (when the half day will be managed, for the moment I put by default at 12:00:00)
		...
		// TODO I do not take into account if there is a non-working day (example: a Sunday)
	...
	function save(&$PDOdb)
	{
        ...
        if (!empty($conf->global->ASSET_USE_PROJECT_TASK))
		{
			$this->manageProjectTask($PDOdb); // TODO when updating a delivery date on a supplier order, you should remember to use the TAssetWorkstationOF save to update all the task dates (be careful, you will have to exclude them themselves in the search for capacity)
		}
		parent::save($PDOdb);
	}
	...
	//Operating mode and not project task
