						...
						//	var_dump($of->getId(), $of->status, $conf->global->OF_FOLLOW_SUPPLIER_ORDER_STATUS);
							if($of->status != 'CLOSE') {
								if(!empty($conf->global->OF_FOLLOW_SUPPLIER_ORDER_STATUS)) {
									...
									$of->closeOF($PDOdb);//TODO strange to close the OF systematically, add optionally I think
...
//                                    else
//                                    {
                                        // TODO it would also be necessary to recalculate the dates of the other tasks not concerned by the supplier order because it is possible that there will have an impact
                                        // Be careful, this must be done outside of the loops here, because it is necessary to impact all the tasks concerned.
//                                    }
