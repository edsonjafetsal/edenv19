...
class modof extends DolibarrModules
{
	...
	function __construct($db)
	{
		...
		$this->const = array(
			...
			,array('ASSET_MANUAL_WAREHOUSE', 'chaine',0,'Manually ventilate the warehouses on the\'OF',1)
			,array('ASSET_USE_DEFAULT_WAREHOUSE', 'chaine',0,'Specifies whether the destocking / storage behavior must take the warehouse of the configuration or the one defined on each device',1)
			,array('ASSET_DEFAULT_WAREHOUSE_ID_TO_MAKE',0,'Warehouse identifier to manage the stock via the OF (products to be manufactured)',1)
			,array('ASSET_DEFAULT_WAREHOUSE_ID_NEEDED',0,'Warehouse identifier to manage the stock via the OF (products required)',1)
			,array('USE_LOT_IN_OF', 'chaine', 0,'Use batch management in OF',1)
			,array('CREATE_CHILDREN_OF', 'chaine', 1,'Allows you to create child OFs if the components are out of stock ',1)
			,array('ASSET_USE_PROJECT_TASK', 'chaine', 0,'Each workstation associated with an OF will create a task for the associated project',1)
			,array('ASSET_DEFINED_USER_BY_WORKSTATION', 'chaine', 0,'Allow the association of one or more users to be assigned to a workstation on an OF',1)
			,array('ASSET_DEFINED_OPERATION_BY_WORKSTATION', 'chaine', 0,'Allows you to define an operating protocol for each workstation',1)
			,array('ASSET_DEFINED_WORKSTATION_BY_NEEDED', 'chaine', 0,'Allows the composition products to be broken down by workstation',1)
		);
        ...
        */
		...
		$this->rights[$r][1] = 'Read Production Orders';
		$this->rights[$r][3] = 0;
		$this->rights[$r][4] = 'of';
		...
		$r++;
		$this->rights[$r][0] = $this->numero+$r;
		$this->rights[$r][1] = 'Create Production Orders ';
		$this->rights[$r][3] =0;
		$this->rights[$r][4] = 'of';
		...
		$r++;
		$this->rights[$r][0] = $this->numero+$r;
		$this->rights[$r][1] = 'Generate documents';
		$this->rights[$r][3] = 0;
		$this->rights[$r][4] = 'read';

		$r++;
		$this->rights[$r][0] = $this->numero+$r;
		$this->rights[$r][1] = 'See the cost of an OF';
		$this->rights[$r][3] = 0;
		$this->rights[$r][4] = 'of';
		...
		$r++;
		$this->rights[$r][0] = $this->numero+$r;
		$this->rights[$r][1] = 'See the forecasted time on an OF';
		$this->rights[$r][3] = 0;
		$this->rights[$r][4] = 'of';
		...
		$r++;
		$this->rights[$r][0] = $this->numero+$r;
		$this->rights[$r][1] = 'Authorize the deletion of an OF in the "Completed" state"';
		$this->rights[$r][3] = 0;
		$this->rights[$r][4] = 'of';
	...
	function init($options='')
	{
        ...
        $res = $extrafields->addExtraField('fk_of', 'Fabrication order', 'sellist', 0, '', 'projet_task',0,0,'',serialize(array('options'=>array('assetOf:numero:rowid'=>null))));
        $res = $extrafields->addExtraField('fk_product', 'Product to manufacture', 'sellist', 0, '', 'projet_task',0,0,'',serialize(array('options'=>array('product:label:rowid'=>null))));
        $res = $extrafields->addExtraField('of_check_prev', 'To be taken into account for the production forecast', 'boolean', 0, '', 'propal',0,0,'','');
        $res = $extrafields->addExtraField('fk_of', 'ID of the linked OF', 'int', 0, 10, 'stocktransfer_stocktransfer', 0, 0, '', unserialize('a:1:{s:7:"options";a:1:{s:0:"";N;}}'), 0, '', 0, '');
        $res = $extrafields->addExtraField('linked_of', 'OF bound', 'html', 0, 2000, 'stocktransfer_stocktransfer', 0, 0, '', unserialize('a:1:{s:7:"options";a:1:{s:0:"";N;}}'), 0, '', 5, '');
