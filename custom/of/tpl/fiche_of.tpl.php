		...
		<form id="formOF[assetOf.id]" name="formOF[assetOf.id]" action="fiche_of.php" method="POST">
			...
			<table width="100%" class="border">
				...
				[onshow;block=begin;when [rights.show_ws_time]==1]
					<tr><td>[view.langs.transnoentities(EstimatedMakeTime)]</td><td>[assetOf.temps_estime_fabrication;strconv=no] Hour(s)</td></tr>
				[onshow;block=end]
				<tr><td>[view.langs.transnoentities(RealMakeTime)]</td><td>[assetOf.temps_reel_fabrication;strconv=no] Hour(s)</td></tr>
				...
				<tr rel="note">
					<td>[view.editField;strconv=no][view.langs.transnoentities(Referral orders)]</td>
		...
		function refreshDisplay() {
			...
			$(".TAssetOFLineLot").each(function () {
				...
				$(this).autocomplete({
					...
					var inputTarget = $(this).parent().next().find('.multiselect');
