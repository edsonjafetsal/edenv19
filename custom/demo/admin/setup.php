...
/**
 * \file    demo/admin/setup.php
 * \ingroup demo
 * \brief   Demo setup page.
 */
...
require_once '../lib/demo.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "demo@demo"));
...
$arrayofparameters = array(
	'DEMO_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'DEMO_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/demo/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DEMO_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'DEMO_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print load_fiche_titre($langs->trans($page_name), $linkback, 'object_demo@demo');

// Configuration header
$head = demoAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "demo@demo");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DemoSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'DEMO_'.strtoupper($myTmpObjectKey).'_ADDON';
