...
class ExtDirect
{
	...
	public static function getDolError($errorCode, $errors = null, $error = null)
	{
		...
		dol_syslog("ExtDirect::getDolError " . $errorText, LOG_ERR);
	...
	/**
	 * Check if an optional exists and enabled
	 *
	 * @param   Object  $object to check optional from
	 * @param   String  $optional optional to check
	 *
	 * @return String label of optional, empty string if optional not found
	 */
	public static function checkOptionalExist($object, $optional)
	{
		$results = self::readOptionalModel($object);

		if (count($results) > 0) {
			foreach ($results as $row) {
				if ($row->name == $optional) return $row->label;
			}
		}
		return '';
	}
