				...
				foreach ($this->lines as $line) {
					if (!isset($id) || ($id == $line->id)) {
						if ($line->fk_product) {
							...
							// custom mobilid field to show in list
							if (!empty($conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_FIELD) && ExtDirect::checkOptionalExist($myprod, $conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_FIELD)) {
								$myprod->fetch_optionals();
								$mobilid_field = $myprod->array_options['options_'.$conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_FIELD];
							}
							// step increment field
							if (!empty($supplierProduct->product_fourn_price_id)) {
								...
								if (!empty($conf->global->MOBILIDCONNECT_PRODUCT_FOURNISSEUR_PRICE_MOBILID_COUNTSTEP) && ExtDirect::checkOptionalExist($supplierProduct, $conf->global->MOBILIDCONNECT_PRODUCT_FOURNISSEUR_PRICE_MOBILID_COUNTSTEP)) {
									...
									if (!empty($supplierProduct->array_options['options_'.$conf->global->MOBILIDCONNECT_PRODUCT_FOURNISSEUR_PRICE_MOBILID_COUNTSTEP])) {
										$mobilid_countstep = $supplierProduct->array_options['options_'.$conf->global->MOBILIDCONNECT_PRODUCT_FOURNISSEUR_PRICE_MOBILID_COUNTSTEP];
									}
								}
							}
							if (!isset($mobilid_countstep) && !empty($conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_COUNTSTEP) && ExtDirect::checkOptionalExist($myprod, $conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_COUNTSTEP)) {
								$myprod->fetch_optionals();
								$mobilid_countstep = $myprod->array_options['options_'.$conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_COUNTSTEP];
							}
						} else {
							$isFreeLine = true;
						}
						...
						if ($isService || $isFreeLine || !empty($warehouse_id) || ($myprod->stock_reel == 0)) {
							if (($warehouse_id == -1 || $isService || $isFreeLine )) {
								...
								if ($isService) {
									...
									$row->product_weight = ($myprod->weight != '') ? $myprod->weight." ".measuringUnitString(0, "weight", $myprod->weight_units) : null;
									$row->product_surface = ($myprod->surface != '') ? $myprod->surface." ".measuringUnitString(0, "surface", $myprod->surface_units) : null;
									$row->product_volume = ($myprod->volume != '') ? $myprod->volume." ".measuringUnitString(0, "volume", $myprod->volume_units) : null;
								}
								...
								$row->mobilid_field = $mobilid_field;
								$row->mobilid_countstep = $mobilid_countstep;
								array_push($results, $row);
							} else {
