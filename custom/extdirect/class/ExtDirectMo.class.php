...
class ExtDirectMo extends Mo
{
	...
	private function getLineData(MoLine $object, Mo $mo, ExtDirectProduct $product = null, $photoSize = '')
	{
		...
		if (!empty($conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_FIELD) && ExtDirect::checkOptionalExist($product, $conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_FIELD)) {
			$product->fetch_optionals();
			$data->mobilid_field = $product->array_options['options_'.$conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_FIELD];
		}
		// step increment field
		if (!empty($conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_COUNTSTEP) && ExtDirect::checkOptionalExist($product, $conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_COUNTSTEP)) {
			$product->fetch_optionals();
			$data->mobilid_countstep = $product->array_options['options_'.$conf->global->MOBILIDCONNECT_PRODUCT_MOBILID_COUNTSTEP];
		}
