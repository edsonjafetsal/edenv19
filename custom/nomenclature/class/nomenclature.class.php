	...
	function setPrice(&$PDOdb, $qty_ref, $fk_object, $object_type,$fk_origin = 0,$fk_product = 0) {
	    ...
	    switch ($object_type)
        {
		   ...
		   case 'commande':
			  ...
			  // TODO the "invoice" case seems to exist on a triggering of LINEBILL_INSERT trigger, it would potentially be necessary to go back to the original order then to the original propal to recover the custom coefs
        }
		...
		foreach($this->TNomenclatureDet as &$det ) {
			...
			// Coefficient applied to the cost price (margin coefficient per line)
			$coef2 = 1;
		...
		foreach($this->TNomenclatureWorkstation as &$ws) {
			...
			if(!empty($conf->global->NOMENCLATURE_ACTIVATE_DETAILS_COSTS) && !empty($conf->of->enabled)) {
			 	list($ws->nb_hour_calculate_of, $ws->calculate_price_of) = $ws->getPrice($PDOdb, $coef_qty_price, 'OF'); // [FIXME] - do I have to take the coef into account in this case to be applied?
...
//var_dump($n);
	...
	public function getRecursiveDetInfos($TNomenclatureDet, $coef = 1, $recursive = false, $qtyligne = 1)
	{
		...
		foreach($TNomenclatureDet as &$det) {
			...
			if ($recursive) {
				// TODO get arbo from each line $ n_det to apply the same method
				...
				//  TODO recursively retrieve data from workstations
			}
			else
			{
				$n=new TNomenclature;
    ...
    function getDetails($qty_ref = 1) {
        ...
        if($qty_ref != $this->qty_reference) {
            // TODO computation on the unbreakable one probably not useful because $ qty_ref should be a multiple of $ this-> qty_reference
	...
	/**
	 * @return array :returns an array containing in key the fk_product and in value the type of this product in the nomenclature
	 */
	...
	/*
	* Function to know if a product of a certain type does not have other children of the same type
...
class TNomenclatureDet extends TObjetStd
{
	...
	/*
	 * Returns the unit price according to the quantity ordered
	 */
    function getSupplierPrice(&$PDOdb, $qty = 1, $searchforhigherqtyifnone=false, $search_child_price=true, $force_cost_price=false, $best_one = false) {
		...
		//If no supplier price is available
	...
	/**
     * returns the default nomenclature of the product in the line
     * TODO is a useless method !!!
	 *
	 * @param      $PDOdb
