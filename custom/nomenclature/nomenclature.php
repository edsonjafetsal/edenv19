...
function _show_product_nomenclature(&$PDOdb, &$product, &$object) {
	...
	llxHeader('',$langs->trans('Bill of Materials'));
	...
	$titre=$langs->trans('Bill of Materials');
	...
	echo $liste->render($PDOdb, $sql, array(
		...
		,'link'=>array(
			'Id'=>'<a href="'.dol_buildpath('/nomenclature/nomenclature.php?fk_product=@val@',1).'">'.img_picto($langs->trans('Nomenclature'),'object_list').' '.$langs->trans('Bill of Material').'</a>'
		)
		,'liste'=>array(
		...
		,'title'=>array(
			...
			,'qty'=>$langs->trans('Qty')
			,'Id'=>$langs->trans('Bill of Material')
		)
		,'eval'=>array(
...
function _fiche_nomenclature(&$PDOdb, &$n,&$product, &$object, $fk_object=0, $object_type='product', $qty_ref=1, $readonly=false) {
	...
	print $langs->trans('BOM').' n°'.$n->getId();
	print ' '.$n->title;
    ...
    <table class="liste" width="100%" id="nomenclature-<?php echo $n->getId(); ?>"><?php
    	if($object_type == 'product') {
	        ?><tr class="liste_titre">
	            <td class="liste_titre"><?php echo $langs->trans('Bill of Material').' '.$langs->trans('numberShort').$n->getId(); ?></td>
                       ...
                       foreach($TNomenclatureDet as $k=>&$det) {
                           ...
                           <tr class="<?php echo $class ?>" rowid="<?php echo $det->getId(); ?>" id="line_<?php echo $det->getId(); ?>">
                               ...
                               ?>
								   ...
								   <td nowrap><?php echo "Find Missing Values"; ?>
...
//					   var_dump($ws->getId(), $ws);
                       ...
                       <tr class="<?php echo $class ?>"  id="nomenclature-ws-item-k-<?php echo $k; ?>" rowid="<?php echo $ws->getId(); ?>">
						   ...
						   ?></td>
                           <td>
                           	<?php echo 'Find Missing Values' //original value rey echo $formCore->combo('', 'TNomenclatureWorkstation['.$k.'][code_type]', TNomenclatureDet::getTType($PDOdb, false, 'workstation'), $ws->code_type, 1, '', '', 'select_coef'); ?>
                           </td>
