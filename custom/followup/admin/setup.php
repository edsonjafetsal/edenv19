...
/**
 * \file    followup/admin/setup.php
 * \ingroup followup
 * \brief   Followup setup page.
 */
...
require_once '../lib/followup.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "followup@followup"));
...
$arrayofparameters = array(
	'FOLLOWUP_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'FOLLOWUP_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/followup/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'FOLLOWUP_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'FOLLOWUP_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$page_name = "FollowupSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_followup@followup');

// Configuration header
$head = followupAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "followup@followup");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("FollowupSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'FOLLOWUP_'.strtoupper($myTmpObjectKey).'_ADDON';
