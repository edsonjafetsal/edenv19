<?php
/////// CONEXIÓN A LA BASE DE DATOS FOLLOWUP /////////
///
///
...
if ($num > 0) {
	...
	while ($CABECERACO = $cabecerasCO->fetch_assoc()) {
		...
		$query2 .= " AND a.datep < CURDATE() + INTERVAL 7 - WEEKDAY(CURDATE()) DAY ";  //registros semana actual
		...
		while ($filaAlumnos2 = $buscarAlumnos2->fetch_assoc()) {
			...
			$dateFollowUp->setTime(0, 0, 0); // Establecer la hora a 00:00:00
...
// Verificar si la diferencia es cero (0)
			if ($diferencia->format('%R%a') == 0) {
				// Las fechas son iguales
				$diferenciaDias = 0;
			} else {
				// Las fechas son diferentes, obtener la diferencia en días
