...
/**
 * \file    hmv/admin/setup.php
 * \ingroup hmv
 * \brief   HMV setup page.
 */
...
require_once '../lib/hmv.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "hmv@hmv"));
...
$arrayofparameters = array(
	'HMV_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'HMV_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/hmv/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'HMV_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'HMV_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print load_fiche_titre($langs->trans($page_name), $linkback, 'object_hmv@hmv');

// Configuration header
$head = hmvAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "hmv@hmv");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("HMVSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'HMV_'.strtoupper($myTmpObjectKey).'_ADDON';
