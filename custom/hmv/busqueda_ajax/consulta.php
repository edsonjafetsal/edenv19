...
if ($buscarAlumnos->num_rows > 0)
{
	...
	while($filaAlumnos= $buscarAlumnos->fetch_assoc())
	{
		...
		'<tr>
		   ...
		   <td><input type="checkbox" id="cbox20" value=""><td>
		 </tr>
		';
		$sql = "SELECT rowid, proc FROM llx_commande_extrafields WHERE fk_object =".$filaAlumnos['id'];
		$resql = $db->query($sql);

		foreach ($resql AS $values){}
		if ($filaAlumnos['proc'] == 0){ print '<td><a class="butAction"'.($conf->use_javascript_ajax ?  : '').'" href="'.DOL_URL_ROOT.'/custom/hmv/hmvindex.php?actionid='.$filaAlumnos['id'].'&amp;actionup=update">'.$langs->trans("0").'</a></td>';}
		elseif ($filaAlumnos['proc'] == 1){ print '<td><a class="butAction"'.($conf->use_javascript_ajax ?  : '').'" href="'.DOL_URL_ROOT.'/custom/hmv/hmvindex.php?actionid='.$value[16].'&amp;actionup=return">'.$langs->trans("1").'</a></td>';}
