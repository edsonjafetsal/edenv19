...
print load_fiche_titre($combinedText, '', 'hmv.png@hmv');
...
//         $('.checkforselect').on('click',function(e){
//             processallorder(e);
//         });
...
function processcolumn(e){
            var checked=$(e).is(':checked');
			var index=parseInt($(e).attr('data-value'));
            var tbl = $('.tablageneral');
			if(checked){
                $(tbl).each(function(i, obj) {
                   $(obj).find('thead tr td:nth-child('+index+')').show();
                   $(obj).find('tbody tr td:nth-child('+index+')').show();
              });
			}
			else{
                $(tbl).each(function(i, obj) {
                    $(obj).find('thead tr td:nth-child('+index+')').hide();
                    $(obj).find('tbody tr td:nth-child('+index+')').hide();
                });
}
  }
function processcolumall(e){
...
//               $('#column_description').prop('checked',false);
//               processcolumn($('#column_description'));
             $('#columnsvisibility').show();
        }
        else{
             $('input[type=\"checkbox\"][name=\"column_table\"]').each(function(i, obj) {
               $(obj).prop('checked', true);
               processcolumn(obj);
             });
             $('input[type=\"checkbox\"][name=\"typesfilter\"]').each(function(i, obj) {
               $(obj).prop('checked', true);
             });

         $('#columnsvisibility').hide();
        }
        $('#column_description').each(function(i, obj) {
  ...
  $.ajax({
    ...
    function Datos()
    {
      ...
      $.ajax({
        url  : 'consulta.php',
        type : 'post',
        data: {cliente :cliente, producto:producto, cate:cate, sales:sales, date1:date1, date2:date2, prc:prc,
        isproduct:isproduct,isservice:isservice,isdropshipped:isdropshipped,isnotdropshipped:isnotdropshipped},
      }).done(function(resultado){
            $('#tabla_resultado').html(resultado);
            processcolumall(true);
        })
    ...
    function resetbtnhvm(){
		$('#date1').val('2021-01-01');
		$('#date2').val('2022-01-01');
        $('#cliente').val('');
        $('#product').val('');
        $('#rep').val('');
        $('#cate').val('');
        $('#prc').val('');
        processcolumall(false);
...
print '<table class="tablacabecera">
			<tr>
				...
				<td><div class="row"><table style="margin-left: 10px;">
				<tbody><tr><td>Product Type</td><td>DropShipped</td></tr></tbody></table>
				<div class="col-lg-3">
					  <input class="check" checked type="checkbox" id="producttype" name="typesfilter" style="-ms-transform: scale(1.5);-moz-transform: scale(2);-webkit-transform: scale(1.5);-o-transform: scale(1.5); background-color: #c4c6ca;">   Product<br> </div>  <div class="col-lg-3">
					  <input class="check" checked  type="checkbox" id="servicetype" name="typesfilter" style="-ms-transform: scale(1.5);-moz-transform: scale(2);-webkit-transform: scale(1.5);-o-transform: scale(1.5); background-color: #c4c6ca;">   Service<br> </div><div class="col-lg-3">
					  <input class="check" checked type="checkbox" id="dropshipped" name="typesfilter" style="-ms-transform: scale(1.5);-moz-transform: scale(2);-webkit-transform: scale(1.5);-o-transform: scale(1.5); background-color: #c4c6ca;">  by Hielscher<br> </div><div class="col-lg-3">
					  <input class="check" checked type="checkbox" id="nodropshipped" name="typesfilter" style="-ms-transform: scale(1.5);-moz-transform: scale(2);-webkit-transform: scale(1.5);-o-transform: scale(1.5); background-color: #c4c6ca;">  by Hitechtrader<br> </div></div></td>
			</tr>';
print '	<tr>';
print '		<td>';
...
$sql = "SELECT  u.rowid AS rowid,gu.proc as processed, CASE(gu.proc) WHEN '1' THEN 'Yes' WHEN '0' THEN 'No' END AS proc FROM llx_commande AS u LEFT JOIN  llx_commande_extrafields AS gu ON u.rowid = gu.fk_object GROUP BY gu.proc";
$resql = $db->query($sql);
print '<div>';
...
foreach ($resql as $pcr) {
	print "<option value = " . $pcr['processed'] . " >" . $pcr['proc'] . "</option>";
}
print '</select>';

print '		</td>';
print ' <td><br>
					<div id="columnsvisibility"><h6>Hide Columns</h6><form id="formcolumn"><div class="row"><div class="col-lg-4">
					<div class="form-check form-switch">
					  <input class="form-check-input" data-value=1 name="column_table" type="checkbox" id="colum_product" checked onclick="processcolumn(this)" >
					  <label class="form-check-label" for="colum_product">Product</label>
					</div>
					<div class="form-check form-switch">
					  <input class="form-check-input" data-value=3 name="column_table" type="checkbox" id="column_description"  onclick="processcolumn(this)" >
					  <label class="form-check-label" for="column_description">Description</label>
					</div><div class="form-check form-switch">
					  <input class="form-check-input" data-value=4  name="column_table" type="checkbox" id="column_lotShipment" checked onclick="processcolumn(this)" >
					  <label class="form-check-label" for="column_lotShipment">Lot</label>
					</div>
					</div>
					<div class="col-lg-4"><div class="form-check form-switch">
					  <input class="form-check-input" data-value=6 name="column_table" type="checkbox" id="colum_itemprice" checked onclick="processcolumn(this)" >
					  <label class="form-check-label" for="colum_itemprice">Selling Price</label>
					</div>
					<div class="form-check form-switch">
					  <input class="form-check-input" data-value=7  name="column_table" type="checkbox" id="column_costprice" checked onclick="processcolumn(this)" >
					  <label class="form-check-label" for="column_costprice">Buying Price</label>
					</div>
					</div></div></form></div></td>';
