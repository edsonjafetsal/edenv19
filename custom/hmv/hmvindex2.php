...
if (! empty($conf->hmv->enabled) && $user->rights->hmv->read)
{
	...
	$sql.= " FROM ".MAIN_DB_PREFIX."hmv_myobject as s";
	//if (! $user->rights->societe->client->voir && ! $socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
...
$(document).ready(function(){
  $('#product').on('change',function(){
    ...
    $.ajax({
       type: 'POST',
       url : 'hmvindex.php',
       data: {value: valor, cat: valor1, cli: valor2, rep: valor3},
...
print '<label class ="form-label"> DATE</label><input type="date" class="date-range" id="date1" > <label class ="form-label">TO  DATE</label><input type="date" class="date-range" id="date1" ><br>';

$sql = "SELECT rowid, nom FROM llx_societe ";
$resql = $db->query($sql);
print '<label class ="form-label">Customer</label>';
print '<select class="form-select" style="height:20px; width:230px" aria-label="size 3 select example" name = "cliente">';
...
print '<select  class="form-select" style="height:20px; width:220px" aria-label="Default select example" id = "product" name = "product">';
print '<option value = "$b" selected>         </option>';
for ($i=0; $i < $num; $i++)
{
...
print '<select class="form-select" style="height:20px; width:160px" aria-label="Default select example" name = "cat">';
print	"<option selected> </option>";
...
$resql = $db->query($sql);
print '<div>';
print '<label class ="container">Sales Representative</label>';
print '<select class="form-select" style="height:20px; width:90px" aria-label="Default select example" name = "rep" >';
print	"<option selected> </option>";
...
$sql = "SELECT CONCAT(u.firstname,' ',u.lastname) AS salesrep, (cod.buy_price_ht*cod.qty) AS extension, FORMAT(cod.total_ht, 2) AS total_line, co.ref AS co, p.ref AS product, ex.ref AS shipment, eba.batch AS lot_shipment, CASE(p.tobatch) WHEN '0' THEN 'NO' WHEN '1' THEN 'YES' END AS is_lot, soc.nom AS third,  p.description AS prod_desc, FORMAT(cod.subprice, 2) AS sell_pri, FORMAT(cod.buy_price_ht, 2) AS buy_pri, co.date_commande AS co_date, cod.qty As qty, cat.label AS categorie, CONCAT(ROUND(100-(((cod.buy_price_ht*cod.qty)* 100)/cod.total_ht) , 0), '%') AS margin2, co.rowid, coex.proc, CASE(coex.proc) WHEN'0' THEN 'NO' WHEN '1' THEN 'YES' END AS prc";
	...
	$resql = $db->query($sql);
	//print '<pre>product ';print_r($sql);print '</pre>';
if ($resql)
{
$a1=0;
$b=0;
...
//print '<pre>product ';print_r($product);print '</pre>';
//print '<pre>catego ';print_r($cat);print '</pre>';
//print '<pre>rep ';print_r($rep);print '</pre>';
...
foreach($datos as $value)
{
	print '<tr>';
		...
		if (!$check16){ print'<td><input type="checkbox" id="cbox20" value=""><td>';}
	...
	if ($resql)
{
	...
	setEventMessages($values['2'].' : ', 'Order Processed', 'mesgs');
...
if ($actionup == 'return') {
	...
	if ($resql)
	{
		...
		setEventMessages($values['2'].' : ', 'Order Unprocessed', 'mesgs');
