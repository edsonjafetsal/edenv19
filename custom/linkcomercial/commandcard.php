...
require_once DOL_DOCUMENT_ROOT . '/custom/linkcomercial/class/PrintLineStock.class.php';
...
$origincomande = new Commande($db);
$origincomande->fetch($originid);
// Load object
if ($id > 0 || !empty($ref)) {
...
//			$error++;
		...
		if (!$error) {
			$db->begin();
//TODO Razmi Primero hacer loop con los diferentes vendors y Usar parametros de $origincomande al eliminar cabecera
			...
			if (!empty($origin) && !empty($originid)) {
				...
				$vendorskus = array();
				$cant = Count($selectedLines);
				$vendorskusvalues = array();
				for ($i = 0; $i < $cant; $i++) {
					if (!in_array($lines[$i]->id, $selectedLines)) continue; // Skip unselected lines
					if (GETPOST("fourn" . $lines[$i]->fk_product, 'int') > 0) {
						$vendorskusvalues[] = GETPOST("fourn" . $lines[$i]->fk_product, 'int');
					}
				}
				//TODO Razmi los skus
				$vendorskusvalues = array_unique($vendorskusvalues);
				foreach ($vendorskusvalues as $skuvalues) {
					// Creation commande
					$object->ref_supplier = $origincomande->ref;
					$object->socid = $skuvalues;
					$object->cond_reglement_id = $origincomande->cond_reglement_id;
					$object->mode_reglement_id = $origincomande->mode_reglement_id;
					$object->fk_account = $origincomande->fk_account;
					$object->note_private = $origincomande->note_private;
					$object->note_public = $origincomande->note_public;
					$object->date_livraison = $origincomande->date_livraison; // deprecated
					$object->delivery_date = $origincomande->delivery_date;
					$object->fk_incoterms = $origincomande->fk_incoterms;
					$object->location_incoterms = $origincomande->location_incoterms;
					$object->multicurrency_code = $origincomande->multicurrency_code;
					$object->multicurrency_tx = $origincomande->multicurrency_tx;
					$object->fk_project = $origincomande->fk_project;
						...
						if ($id > 0) {
							...
							if ($result > 0) {
								...
								for ($i = 0; $i < $num; $i++) {
									if (!in_array($lines[$i]->id, $selectedLines)) continue; // Skip unselected lines
									$fourned = $i + 1;
									//TODO Razmi hacer loop con tercero proveedores seleccionados en sku
									if (GETPOST("fourn" . $lines[$i]->fk_product, 'int') == $skuvalues) {
//										if (empty($lines[$i]->subprice) || $lines[$i]->qty <= 0 || !in_array($lines[$i]->id, $selectedLines))
//											continue;
										if (empty($lines[$i]->subprice) || $torder[$i] <= 0 || !in_array($lines[$i]->id, $selectedLines))
											continue;
										...
										if ($origin == "commande") {
											...
											//alan
											$nuevasVariables = array();

// Itera sobre $_POST
											foreach ($_POST as $clave => $valor) {
												// Verifica si la clave coincide con el patrón
												if (strpos($clave, "fourn") === 0) {
													// Extrae las XXX de la clave
													$xxx = substr($clave, 5);

													// Agrega el valor al array utilizando las XXX como clave
													if($valor >0){
														$nuevasVariables[$xxx] = $valor;
													}

												}
											}
//fin alan
											foreach ($nuevasVariables as $xxx => $valor) {
												// Compara el valor de $xxx con $otraVariable
												if ($xxx == $lines[$i]->fk_product) {
													// Si hay una coincidencia, realiza la acción deseada

													$Rowidnuevovendor =$valor;
													// Aquí puedes realizar más acciones si es necesario
												}
											}

											/*
											 * obtencion del fksoc   x rowid
											 * */
											$sqlObtencionFKsoc='select fk_soc from '.MAIN_DB_PREFIX.'product_fournisseur_price where  rowid ='.$Rowidnuevovendor;
											$resultFKsoc = $db->query($sqlObtencionFKsoc);
											$numFKsoc = $db->num_rows($resultFKsoc);
											if($numFKsoc >0){
												$nuevovendor=$db->fetch_object($resultFKsoc);
											}

											/*
											 *fin  obtencion del fksoc   x rowid
											 * */
									 ...
									 * */
											...
											//$result = $productsupplier->find_min_price_product_fournisseur($lines[$i]->fk_product, $lines[$i]->qty, $nuevovendor->fk_soc);
											$result = $productsupplier->find_min_price_product_fournisseur($lines[$i]->fk_product, $torder[$i], $Rowidnuevovendor);




											//$result = $productsupplier->find_min_price_product_fournisseur($lines[$i]->fk_product, $torder[$i], $srcobject->socid);
											if ($result > 0) {
												$ref_supplier = $productsupplier->ref_supplier;
										...
										$supplierpriceid = GETPOST('fourn' . $srcobject->lines[$i]->fk_product);
										$idprod = $productsupplier->get_buyprice($supplierpriceid, $torder[$i]);
										$preciounitario = 0;
										if ($productsupplier->fourn_pu > 0) {
											$preciounitario = $productsupplier->fourn_pu;
										} else {
											$preciounitario = $lines[$i]->subprice;
										}
										$prod = new Product($db);
										$prod->get_buyprice(0, $torder[$i], $lines[$i]->fk_product, 'none', 0);
										$preciounitario = $prod->fourn_pu;
										                       // Unit price supplier price set by get_buyprice
										if(!$preciounitario) $preciounitario=$pu_ht;
										$result = $object->addline(
											$desc,
											$preciounitario,
											$torder[$i],
...
//TODO Razmi los no skus
				$cant = Count($selectedLines);
				$vendornoskusvalues = array();
				for ($i = 1; $i <= $cant; $i++) {
					if (!in_array($lines[$i]->id, $selectedLines)) continue; // Skip unselected lines
					if (GETPOST("fourn" . $lines[$i]->fk_product, 'int') == 0) {
						$vendornoskusvalues[] = GETPOST("fourn" . $lines[$i]->fk_product, 'int');
					}
				}
				$vendornoskusvalues = array_unique($vendornoskusvalues);
				foreach ($vendornoskusvalues as $skuvalues) {
					// Creation commande
					$object->ref_supplier = $origincomande->ref;
					$object->socid = $origincomande->socid;
					$object->cond_reglement_id = $origincomande->cond_reglement_id;
					$object->mode_reglement_id = $origincomande->mode_reglement_id;
					$object->fk_account = $origincomande->fk_account;
					$object->note_private = $origincomande->note_private;
...
//					if ($ret < 0) $error++;
			...
			if ($error) {
				...
				$db->commit();
				PropagateContacts::propagate($origincomande, $object);
				$origincomande->array_options["options_dropship"] = 1;
				$origincomande->insertExtraFields();
				if ($result < 0) $error++;

			}
		}

		header('Location: ' . DOL_URL_ROOT . '/fourn/commande/list.php?statut=0');
		exit;
...
if ($action == 'create') {
	...
	if (!empty($origin) && !empty($originid)) {
		...
		$objectsrc = new PrintLineStock($db);
	...
	if ($socid > 0)
	{
		...
		$formtiers .= '<input type="hidden" name="originid" value="'.$originid.'">' . PHP_EOL;
		...
		print'<span id="modify_commande_tparty" class="fas fa-pencil-alt" style=" color: #444;cursor: pointer;margin-right: 10px" title="Modify Third Party"></span>';
...
//TODO Razmi cambiar por lo nuevo de stocks
		$objectsrc->printOriginLinesList('', $selectedLines);
		//include_once DOL_DOCUMENT_ROOT.'/custom/linkcomercial/scripts/productstock.php';
		print '</table>';
	}
	print "</form>\n";
		...
		if ($object->multicurrency_code != $conf->currency || $object->multicurrency_tx != 1) {
			...
			if ($action == 'editmulticurrencyrate' || $action == 'actualizemulticurrencyrate') {
				...
				if ($object->statut == $object::STATUS_DRAFT && $object->multicurrency_code && $object->multicurrency_code != $conf->currency) {
					...
					print '<a href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&action=actualizemulticurrencyrate">' . $langs->trans("ActualizeCurrency") . '</a>';
					print '</div>';
				}
			}
			print '</td></tr>';
	...
	if ($action == 'editdate_livraison') {
		print '<form name="setdate_livraison" action="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '" method="post">';
		print '<input type="hidden" name="token" value="' . newToken() . '">';
	...
	print '	<form name="addproduct" id="addproduct" action="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . (($action != 'editline') ? '#addline' : '#line_' . GETPOST('lineid')) . '" method="POST">
		...
		if (empty($reshook)) {
			...
			if ($object->statut == 0 && $num > 0) {
				if ($usercanvalidate) {
					...
					print '<a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=valid">';
			...
			if ($object->statut == CommandeFournisseur::STATUS_VALIDATED) {
				if ($usercanorder) {
					print '<a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card?id=' . $object->id . '&amp;action=reopen">' . $langs->trans("Modify") . '</a>';
				}
			}
			...
			if ($object->statut == CommandeFournisseur::STATUS_VALIDATED) {
				if ($usercanapprove) {
					if (!empty($conf->global->SUPPLIER_ORDER_3_STEPS_TO_BE_APPROVED) && $object->total_ht >= $conf->global->SUPPLIER_ORDER_3_STEPS_TO_BE_APPROVED && !empty($object->user_approve_id)) {
						...
						print '<a class="butAction"	href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=approve">' . $langs->trans("ApproveOrder") . '</a>';
					}
				} else {
				...
				if ($object->statut == CommandeFournisseur::STATUS_VALIDATED) {
					if ($usercanapprovesecond) {
						if (!empty($object->user_approve_id2)) {
							...
							print '<a class="butAction"	href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=approve2">' . $langs->trans("Approve2Order") . '</a>';
						}
					} else {
			...
			if ($object->statut == CommandeFournisseur::STATUS_VALIDATED) {
				if ($usercanapprove || $usercanapprovesecond) {
					print '<a class="butAction"	href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=refuse">' . $langs->trans("RefuseOrder") . '</a>';
				} else {
			...
			if (empty($user->socid)) {
				if (in_array($object->statut, array(CommandeFournisseur::STATUS_ACCEPTED, 3, 4, 5)) || !empty($conf->global->SUPPLIER_ORDER_SENDBYEMAIL_FOR_ALL_STATUS)) {
					if ($usercanorder) {
						print '<a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&action=presend&mode=init#formmailbeforetitle">' . $langs->trans('SendMail') . '</a>';
					}
				}
			}
			...
			if (in_array($object->statut, array(CommandeFournisseur::STATUS_ACCEPTED))) {
				...
				if (!$buttonshown && $usercanapprove) {
					if (empty($conf->global->SUPPLIER_ORDER_REOPEN_BY_APPROVER_ONLY)
						...
						print '<a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=reopen">' . $langs->trans("Disapprove") . '</a>';
						$buttonshown++;
					}
				}
			...
			if (in_array($object->statut, array(3, 4, 5, 6, 7, 9))) {
				if ($usercanorder) {
					print '<a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=reopen">' . $langs->trans("ReOpen") . '</a>';
				}
			}
			...
			if ($object->statut == CommandeFournisseur::STATUS_ACCEPTED) {
				if ($usercanorder) {
					print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card?id=' . $object->id . '&amp;action=makeorder#makeorder">' . $langs->trans("MakeOrder") . '</a></div>';
				} else {
			...
			if ($object->statut == CommandeFournisseur::STATUS_ORDERSENT || $object->statut == CommandeFournisseur::STATUS_RECEIVED_PARTIALLY) {
				if ($usercanreceived) {
					print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=classifyreception#classifyreception">' . $langs->trans("ClassifyReception") . '</a></div>';
				}
			}
			...
			{
				if (empty($conf->facture->enabled)) {
					print '<a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=classifybilled">' . $langs->trans("ClassifyBilled") . '</a>';
				} else {
			...
			{
				print '<a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=webservice&amp;mode=init">' . $langs->trans('CreateRemoteOrder') . '</a>';
			}

			// Clone
			if ($usercancreate) {
				print '<a class="butAction" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;socid=' . $object->socid . '&amp;action=clone&amp;object=order">' . $langs->trans("ToClone") . '</a>';
			}

			// Cancel
			if ($object->statut == 2) {
				if ($usercanorder) {
					print '<a class="butActionDelete" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=cancel">' . $langs->trans("CancelOrder") . '</a>';
				}
			}

			// Delete
			if (!empty($usercandelete) || ($object->statut == CommandeFournisseur::STATUS_DRAFT && !empty($usercancreate))) {
				print '<a class="butActionDelete" href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&amp;action=delete&amp;token=' . newToken() . '">' . $langs->trans("Delete") . '</a>';
			}
		}
...
//			}
			...
			if ($mode == "init") {
				//Table/form header
				print '<table class="border centpercent">';
				print '<form action="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '" method="post">';
				print '<input type="hidden" name="token" value="' . newToken() . '">';
