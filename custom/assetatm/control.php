<?php
	...
	switch($action) {
		case 'view':
			if ($id <= 0) header('Location: '.dol_buildpath('/assetatm/list_control.php',2));
		...
		case 'delete':				
			...
			header('Location: '.dol_buildpath('/assetatm/list_control.php'));
	...
	print $TBS->render('./tpl/control.tpl.php', 
		...
		,array(
			...
			,'view'=>array(
				...
				,'editValue'=>$editValue
				,'type'=>$control->type
				,'url'=>dol_buildpath('/assetatm/control.php', 2)
			)
		)
	);
...
function _liste_valeur(&$PDOdb, $fk_control, $type)
{
	...
	$sql = 'SELECT rowid, value 
			FROM '.MAIN_DB_PREFIX.'assetatm_control_multiple cm
