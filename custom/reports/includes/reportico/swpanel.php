		...
		switch($this->panel_type)
		{
			...
			case "PROJECTITEM":
				if ( $this->text != ".." && $this->text != "admin" )
				{
					...
					$this->query->projectitems[] = array (
						"label" => $this->text,
						"url" => $this->query->get_action_url()."?".$forward."execute_mode=MENU&project=".$this->program."&amp;reportico_session_name=".reportico_session_name().(version_compare(DOL_VERSION, 13) >= 0? "&token=".newToken():'')
							);
				}
				break;
			...
			case "MENUITEM":
				...
				$this->query->menuitems[] = array (
						"label" => $this->text,
						"url" => $this->query->get_action_url()."?".$forward."execute_mode=PREPARE&xmlin=".$this->program."&amp;reportico_session_name=".reportico_session_name().(version_compare(DOL_VERSION, 13) >= 0? "&token=".newToken():'')
							);
				break;
