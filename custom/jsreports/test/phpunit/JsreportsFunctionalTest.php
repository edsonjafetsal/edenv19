...
/**
 * \file    test/phpunit/JsreportsFunctionalTest.php
 * \ingroup jsreports
 ...
 *
 * @package Testjsreports
 */
class JsreportsFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modJsreports')->click();
		} else {
			// Disable the module
			$this->byHref('modJsreports')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/jsreports/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('jsreports/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/jsreports/admin/about.php');
		$this->authenticate();
		return $this->assertContains('jsreports/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('jsreportswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modJsreports_JsreportsTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modJsreports_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
