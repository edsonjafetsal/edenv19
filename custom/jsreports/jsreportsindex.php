...
llxHeader("", $langs->trans("JsreportsArea"));
...
<tr>
	<td>&nbspOrder Date:
	<td>&nbsp;<input type="date" style="height:20px; width:100px" name= "date1" id="date1" class="form-control" value= '.date("Y-m-d").'> &nbsp;to <input type="date" style="height:20px; width:100px" name= "date2" id="date2" class="form-control" value= '.date("Y-m-d").'></td>
	<td>&nbsp;</td>
	<td>&nbsp;</td>
	<td>&nbsp;</td>
</tr>
...
<tr>
	<td>&nbsp;Raw Material</td>
	<td>&nbsp;<?php  print getSelect('nomenclature', '');?></td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
</tr>
...
<section id="tabla_resultado">
	...
	//	document.getElementById("select_c_product_nature").onchange= function() {reporte()};
		...
		function reporte() {
			...
			$.ajax({
				...
				data: {societe: societe,commande: commande,facture: facture,product: product,categorie: categorie,
					...
					nomenclature: nomenclature ,workstation: workstation, date2: date2 },
		...
		function generarbom2() {
			...
			$("input[type=checkbox]:checked").each(function(){
				arraycommande.push($(this).attr('value'));
			});
			$.ajax({
