	...
	if ((!GETPOST('getcustomercode') && !GETPOST('getsuppliercode'))
		...
		if (!$error) {
			if ($action == 'add') {
				...
				$object->array_options["options_phone_extension"] = $_REQUEST["extrarow-societe_phone_extension"];
...
$extracss = '';
$extrajs = array(
    '/societe/js/jquery.mask.js',
    '/societe/js/ponenumbermask.js'
);

llxHeader('', $title, '', '', '', '', $extrajs, '');
		...
		if (empty($reshook)) {
			...
			*/
			...
			document.addEventListener('DOMContentLoaded', function() {
				// Selecciona el elemento del campo de entrada por su ID
				var phoneExtensionInput = document.getElementById('options_phone_extension');
				// Verifica si el elemento del campo de entrada existe antes de intentar ocultarlo
				if (phoneExtensionInput) {
					// Oculta el campo de entrada estableciendo su estilo display en 'none'
					phoneExtensionInput.style.display = 'none';
				}

				// Selecciona el elemento de la etiqueta asociada por su contenido de texto
				var phoneExtensionLabel = Array.from(document.getElementsByTagName('td')).find(td => td.textContent === 'Phone Extensions');
				// Verifica si el elemento de la etiqueta asociada existe antes de intentar ocultarlo
				if (phoneExtensionLabel) {
					// Oculta la etiqueta asociada estableciendo su estilo display en 'none'
					phoneExtensionLabel.style.display = 'none';
				}
			});
		</script>";

			// Phone Extension 
			print '<td>' . $form->editfieldkey('Phone Ext', 'phone_extension', '', $object, 0) . '</td>';
			print '<td>';
			print img_picto('', 'phone-volume');
			print '</td><td><input type="text" class="flat maxwidth75 maxwidthonsmartphone extrarow-societe_phone_extension" name="extrarow-societe_phone_extension" id="extrarow-societe_phone_extension" value="' . dol_escape_htmltag($object->array_options["options_phone_extension"]) . '"></td>';
			print '</tr>';
		...
		if ($socid) {
			...
			if (empty($reshook)) {
				...
				document.addEventListener('DOMContentLoaded', function() {
					...
					var phoneExtensionInput = document.getElementById('options_phone_extension');
					// Verifica si el input existe antes de intentar borrar su valor
					if (phoneExtensionInput) {
						// Borra el valor del input estableciendo su propiedad value en una cadena vacía
						phoneExtensionInput.value = '';
					}
				});
			</script>";
				print "<script>
