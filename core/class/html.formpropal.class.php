...
class FormPropal
{
	...
	public function selectProposalContcat($selected = '', $short = 0, $excludedraft = 0, $showempty = 1, $mode = 'customer', $htmlname = 'propal_contact', $tipe='')
	{
		global $langs, $db;

		$sql  =" SELECT CONCAT(t.firstname, ' ', t.lastname ) AS label, t.rowid, ec.statut as statuslink, ";
		//$sql  =" SELECT CONCAT(t.firstname, ' ', t.lastname ) AS label, ec.rowid, ec.statut as statuslink, ";
		//$sql .=" ec.fk_socpeople as id, ec.fk_c_type_contact, t.rowid as SocPeopleid, t.rowid as id, ";
		$sql .=" ec.fk_socpeople as id, ec.fk_c_type_contact, t.rowid as SocPeopleid, ec.rowid as id, ";
		$sql .=" t.fk_soc as socid, t.statut as statuscontact,  ";
		$sql .=" t.civility as civility, t.lastname as lastname,  ";
		$sql .=" t.firstname, t.email, tc.source, tc.element, tc.code as code, tc.libelle ";
		$sql .=" FROM llx_c_type_contact tc, llx_element_contact ec  ";
		$sql .=" LEFT JOIN llx_socpeople t on ec.fk_socpeople = t.rowid  ";
		$sql .=" WHERE 1=1";
		$sql .=" 			AND ec.fk_c_type_contact=tc.rowid ";
		$sql .=" 			and CONCAT(t.firstname, ' ', t.lastname ) != 'Accounts Payable' ";
		$sql .=" 			and CONCAT(t.firstname, ' ', t.lastname ) != 'Account Payable' ";
		$sql .=" 			AND tc.element='propal' ";
		$sql .=" 			AND tc.source = 'external' AND tc.active=1 ";
		$sql .=" 			AND   tc.libelle  = '".$tipe."'";
		$sql .="   ";
		$sql .=" ORDER BY label ASC ";

			dol_syslog(get_class($this)."::selectProposalContact", LOG_DEBUG);
			$resql = $this->db->query($sql);
			if ($resql)
			{
				$num = $this->db->num_rows($resql);
				...
				if ($num)
				{
					...
					{
						$obj = $this->db->fetch_object($resql);
						$listofstatus[$obj->SocPeopleid] = array('id'=>$obj->SocPeopleid, 'code'=>$obj->code, 'label'=>$obj->label);
						$i++;
					}
				}
			} else {
