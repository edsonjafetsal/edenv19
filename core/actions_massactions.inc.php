			...
			foreach ($listofobjectref[$thirdpartyid] as $objectid => $objectobj)
			{
				...
				if (empty($sendto)) 	// For the case, no recipient were set (multi thirdparties send)
				{
					...
					} elseif ($objectobj->element == 'facture' && !empty($listofobjectcontacts[$objectid]))
					{
						...
						//edson
						if($conf->global->ModuleTestingContact > 0){
							$code = 'BulkEmail';
							...
							$sqlBulkExist .= " WHERE ec.element_id =".$objectid." AND ec.fk_c_type_contact=tc.rowid AND tc.code = '".$code."' AND tc.element='propal' AND tc.source = 'external' AND tc.active=1 ORDER BY t.lastname asc";
							$resBulk = $db->query($sqlBulkExist);
								$obj = $db->fetch_object($resBulk);
								$objectobj->fetch_thirdparty();
								$objectobj->thirdparty->email=$obj->email;
								$sendto =$obj->email;
						}else{
							$objectobj->fetch_thirdparty();
				...
				if ($_POST['addmaindocfile'])
				{
                     ...
                     * CODIGO MODIFICADO
                     * */
                    $filenameModificado  = scandir($filedir);
                    $filenameModificado = array_diff($filenameModificado, array('.', '..'));
                    $asd=$conf->global->CUSTOM_PDF;
                    if($conf->global->CUSTOM_PDF){  //ALAN
                        $existe=count($filenameModificado);
                        if($existe>0){
                            $filename=$filenameModificado[2];
                        }
