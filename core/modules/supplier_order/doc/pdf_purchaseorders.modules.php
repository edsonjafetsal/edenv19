 ...
 */
class pdf_purchaseorders extends ModelePDFSuppliersOrders
{
	/**
		...
		if ($conf->fournisseur->commande->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					pdf_writelinedesc($pdf, $object, $i, $outputlangs, $descWidth-12, 3, $curX+12, $curY, $hideref, $hidedesc, 1);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if ($this->atleastonediscount)
		{
		 ...
		 if (empty($hidetop))
        {
            $pdf->SetXY($this->postotalht , $tab_top + 1);
            $pdf->MultiCell(30, 2, "Total", '', 'C');
        }
        $pdf->SetXY($this->marge_gauche , (empty($hidetop)?$tab_top + 5:$tab_top));
        $pdf->SetFillColor(202, 202, 202);
        $pdf->MultiCell(11, (empty($hidetop)?$tab_height - 5:$tab_height), "", 0,  'L', 1);
	}
