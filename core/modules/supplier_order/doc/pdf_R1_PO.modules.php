 ...
 */
class pdf_R1_PO extends ModelePDFSuppliersOrders
{
	/**
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						if ($posyafter > ($this->page_hauteur - ($heightforfooter + $heightforfreetext + $heightforinfotot)))	// There is no space left for total+free text
						{
							if ($i == ($nblines - 1))	// No more lines, and no space left to show total, so we create a new page
							{
								...
								if (!empty($tplidx)) $pdf->useTemplate($tplidx);
								if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) $this->_pagehead($pdf, $object, 0, $outputlangs);
								$pdf->MultiCell($widthrecbox+7, $tab_height-5, "", 0,  'L', 1);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		$pdf->line($this->posxup+6, $tab_top, $this->posxup+6, $tab_top + $tab_height);
		if (empty($hidetop))
		{
		...
		if (empty($hidetop))
		{
			if ($this->atleastonediscount)
			{
				...
				$pdf->MultiCell($this->postotalht - $this->posxdiscount + 4, 2, "Add Dct.", '', 'C');
			}
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(30, 2, "Extension", '', 'R');
								    ...
								    // Accountancy piece
		$pdf->line($this->posxdesc +5, $tab_top, $this->posxdesc + 5, $tab_top + $tab_height);
		if (empty($hidetop))
		{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxqty - $this->posxup - 1, 2, $outputlangs->transnoentities("Qty"), '', 'L');$pdf->SetFillColor(202, 202, 202);

		}
		//  discount header
		$pdf->line($this->posxdesc + 181, $tab_top, $this->posxdesc + 181, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			
			$pdf->SetXY($this->posxdesc+168 , $tab_top + 1);
        ...
        $pdf->MultiCell(6, (empty($hidetop)?$tab_height - 5:$tab_height), "", 0,  'L', 1);
	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress)
		{
			...
			$pdf->MultiCell($widthrecbox, 5, "To:", 0, 'L');
			...
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->SetXY($posx + 2, $posy);
			$pdf->MultiCell($widthrecbox, 4,$outputlangs->transnoentities("CustomerCode")." : ".$outputlangs->transnoentities($object->thirdparty->code_fournisseur), 0, 'L');
