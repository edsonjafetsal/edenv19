 ...
 */
class mailing_ejemploAlan extends MailingTargets
{
	public $name = 'ejemploAlan';
...
//		} else {
		...
		$addFilterurl="";
			$urlfiltervalid=GETPOST('filter_url', 'alphanohtml');
			if (isset($urlfiltervalid) && $urlfiltervalid!="") {
				$addFilterurl = " AND se.utmurl ='".$urlfiltervalid."' ";
			...
			$sql .= " WHERE ls.email <> ''";
			$sql .= " AND s.entity IN (".getEntity('societe').")";
			$sql .= " AND ls.email NOT IN (SELECT email FROM ".MAIN_DB_PREFIX."mailing_cibles WHERE fk_mailing=".$mailing_id.")";
...
// FROM llx_user as u WHERE u.entity IN (0,1) ORDER BY statut DESC, lastname ASC
		...
		//tercer select
		$s .= $langs->trans("Hielscher URL").': ';
		$sql = "SELECT  se.fk_object as SocID, se.utmurl as Url ";
		$sql .= " FROM ".MAIN_DB_PREFIX."societe_extrafields as  se ";
		$sql .= " left join ".MAIN_DB_PREFIX."societe as s on se.fk_object=s.rowid ";
		$sql .= " where   1=1 ";
		// $sql.= " AND visible > 0";	// We ignore the property visible because third party's categories does not use this property (only products categories use it).
		$sql .= "AND s.status =1 ";
		$sql .= "AND se.utmurl <> '' ";
...
//		';
		...
		$s .= '<select name="filter_url" class=""  style="width: 80%;" >';  //FILTRO 3
		if ($num) $s .= '<option value="0">&nbsp;</option>';
		else $s .= '<option value="0">'.$langs->trans("ContactsAllShort").'</option>';
		$i = 0;
		while ($i < $num)
		{
			$obj = $this->db->fetch_object($resql);
			global $db;    //COUNT FINTRO 3
			//$sql = "SELECT COUNT(se.utmurl) as total";
			//$sql .=" FROM ".MAIN_DB_PREFIX."societe ls left join ".MAIN_DB_PREFIX."societe_extrafields as se on ls.rowid = se.fk_object";
//			$re2 = $this->db->query($sql);
//			$num2 = $this->db->num_rows($re2);
//			if($num2>0){
//				$Count = $this->db->fetch_object($re2);
//				$totalnum=$Count->total;
//			}else{$totalnum =0;}
			//$s .= '<option value="'.$obj->SocID.'">'.dol_trunc($obj->Url, 38, 'middle');
			$s .= '<option value="'.$obj->Url.'">'.$obj->Url;
