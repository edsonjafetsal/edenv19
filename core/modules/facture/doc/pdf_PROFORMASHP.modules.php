 ...
 */
class pdf_PROFORMASHP extends ModelePDFFactures
{
	 /**
		...
		$arrayidcontact = $object->getIdContact('internal', 'SALESREPFOLL');
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					if (!empty($conf->global->PRODUCT_USE_UNITS))
					{
						...
						$pdf->MultiCell($this->posxdiscount - $this->posxunit +1, 4, $unit, 0, 'C');
					}
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull)
			{
				...
				$pdf->SetFillColor(209, 228, 247);
				$pdf->MultiCell($col2x - $col1x, $tab2_hl,"Total", $useborder, 'L', 1);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($object->type == 2 && !empty($object->fk_facture_source))
		{
			...
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 3, $outputlangs->transnoentities("CorrectionInvoice").' : '.$outputlangs->convToOutputCharset($objectreplaced->ref), '', 'C');
		}

		$posy += 4;
		$pdf->SetXY($posx-5, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell($w, 3, $outputlangs->transnoentities("DateInvoice")." : ".dol_print_date($object->date, "day", false, $outputlangs)." / ", '', 'C');
		...
		if ($object->type != 2)
		{
			...
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 3, "Validity Ending Date"." : ".dol_print_date($object->date_lim_reglement, "day", false, $outputlangs, true), '', 'R');
		}
		...
		if ($showaddress)
		{
			...
			$pdf->RoundedRect($posx, $posy-2, $widthrecbox, $hautcadre+5, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));
			$pdf->SetTextColor(0, 0, 60);
			...
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont('', '', $default_font_size - 2);
			$pdf->SetXY($posx - 13, $posy - 6);
			$pdf->MultiCell($widthrecbox, 5, "Bill To:                                                         Ship to".":", 0, 'L');
			//$pdf->Rect($posx, $posy, $widthrecbox, $hautcadre);

			// Show recipient name
			$pdf->SetXY($posx - 14, $posy - .5);
			$pdf->SetFont('', 'B', $default_font_size-2);
			$pdf->MultiCell($widthrecbox-50, 2, $carac_client_name, 0, 'L');
			//$pdf->MultiCell($widthrecbox, $hautcadre, "", 0, 'R', 1);
			$styles = array('width' => 0.25, 'cap' => 'butt', 'join' => 'miter', 'dash' => '3,3');		
			$pdf->RoundedRect($posx-16, $posy-2, $widthrecbox+18, $hautcadre+5, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));
			$pdf->SetTextColor(0, 0, 60);
			...
			$pdf->SetFont('', '', $default_font_size - 2);
			$pdf->SetXY($posx - 14, $posy-.5);
			$pdf->MultiCell($widthrecbox-45, 4, $carac_client, 0, 'L');
			...
			$carac_client = pdf_build_address($outputlangs, $this->emetteur, $object->contact, ($usecontact ? $object->contact : ''), $usecontact, 'target', $object);
						// Show recipient name
			...
			$pdf->MultiCell($widthrecbox-45, 4, $carac_client, 0, 'L');
									...
									$posy += 24.5;
			$pdf->SetXY($posx-47.5, $posy+7);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 3, $outputlangs->transnoentities("CustomerCode")." : ".$outputlangs->transnoentities($object->thirdparty->code_client), '', 'C');
