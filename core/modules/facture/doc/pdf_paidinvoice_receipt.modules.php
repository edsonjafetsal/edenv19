 ...
 */
class pdf_paidinvoice_receipt extends ModelePDFFactures
{
	 /**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$outputlangs->loadLangs(array("main", "bills", "products", "dict", "companies","productbatch"));
		...
		if ($conf->facture->dir_output)
		{
			...
			if ($object->specimen)
			{
				...
				$file = $dir."/INVOICE_".$objectref."_".$this->emetteur->name."_".$object->thirdparty->name."_".$date_file."_".date("Hi",dol_now()).".pdf";
			}
			if (!file_exists($dir))
			{
				if (dol_mkdir($dir) < 0)
			...
			if (file_exists($dir))
			{
				...
				if (!empty($conf->incoterm->enabled))
				{
					...
					if ($desc_incoterms)
					{
						...
						$pdf->SetFont('', '', $default_font_size - 1);
						$pdf->writeHTMLCell(190, 3, $this->posxdesc - 1, $tab_top - 2, dol_htmlentitiesbr($desc_incoterms), 0, 1);
						global $user;
		...
		$arrayidcontact = $object->getIdContact('internal', 'SALESREPFOLL');
				...
				if (!empty($sale_name))
				{
					...
					// {
						...
						$pdf->SetFont('', '', $default_font_size - 1);
						//$pdf->writeHTMLCell(190, 3, $this->posxdesc - 1, $tab_top - 1,"Case Followed by".": ".$sale_name.' - ' .$sale_email, 0, 1);
						$pdf->MultiCell(190, 3, $langs->trans("Case followed by").": ".$usertmp->getFullName($langs).". Email:".$usertmp->email.". Phone:".$usertmp->office_phone,  0, 1);
				...
				if (!empty($linkedobjects))
				{
					foreach ($linkedobjects as $linkedobject)
					{
						...
						$title = $linkedobject['ref_title']; 
					}		
					if( $title == $outputlangs->transnoentities("RefSending")){
						$exped = new Expedition($this->db);
						$exped->fetch('',$reftoshow);
						$linesex = $exped->lines;
					}
				}
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					$object->lines[$i]->detail_batch = $linesex[$i]->detail_batch;//batch
					...
					// $pdf->writeHTMLCell($this->posxcomment - $this->posxpiece - 0.8, 4, $curX+2, $curY, $i + 1, 0, 1); // Accountancy piece
					$pdf->SetXY( $curX-3, $curY);
					//$pdf->MultiCell($this->posxqty - $this->posxup - 0.8, 3, $i + 1, 0, 'C', 0);
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
					{
						...
						$pdf->SetXY($this->posxtva - 2, $curY);
						$pdf->MultiCell($this->posxup - $this->posxtva + 4, 3, $vat_rate, 0, 'C');
						//$pdf->writeHTMLCell($this->posxcomment - $this->posxpiece - 0.8, 4, $curX+3, $curY, $i + 1, 0, 1); // Accountancy piece
					}
					...
					if ($this->situationinvoice)
					{
						...
						$pdf->MultiCell($this->postotalht - $this->posxprogress + 1, 3, $progress, 0, 'C');
					}
					...
					while ($pagenb < $pageposafter)
					{
						...
						if ($pagenb == 1)
						{
							$this->_tableau($pdf, $tab_top, $this->page_hauteur+6 - $tab_top - $heightforfooter, 0, $outputlangs, 0, 1, $object->multicurrency_code);
						} else {
							$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur+6 - $tab_top_newpage - $heightforfooter, 0, $outputlangs, 1, 1, $object->multicurrency_code);
						}
	...
	protected function _tableau_versements(&$pdf, $object, $posy, $outputlangs, $heightforfooter = 0)
	{
		...
		if ($resql)
		{
			...
			while ($i < $num)
			{
				...
				$pays = $invoice->getListOfPayments();//mod 2byte

				$pdf->SetXY($tab3_posx-30, $tab3_top + $y);
				$pdf->MultiCell(20, 3, dol_print_date($this->db->jdate($obj->datef), 'day', false, $outputlangs, true), 0, 'L', 0);
				$pdf->SetXY($tab3_posx -11, $tab3_top + $y);
				...
				$pdf->SetXY($tab3_posx +24, $tab3_top + $y);
				$pdf->MultiCell(20, 3, $invoice->ref, 0, 'L', 0);
				$pdf->SetXY($tab3_posx +43, $tab3_top + $y);
				$pdf->MultiCell(20, 3, $pays[0]['num'], 0, 'L', 0);//mod 2byte

				$pdf->line($tab3_posx-30, $tab3_top + $y + 3, $tab3_posx + $tab3_width-17, $tab3_top + $y + 3);
	...
	protected function _tableau_versements_header($pdf, $object, $outputlangs, $default_font_size, $tab3_posx, $tab3_top, $tab3_width, $tab3_height)
	{
		...
		$pdf->SetXY($tab3_posx-30, $tab3_top - 4);
		$pdf->MultiCell(60, 3, $title, 0, 'L', 0);

		$pdf->line($tab3_posx-30, $tab3_top, $tab3_posx + $tab3_width-17, $tab3_top);

		$pdf->SetFont('', '', $default_font_size - 4);
		$pdf->SetXY($tab3_posx-30, $tab3_top);
		$pdf->MultiCell(20, 3, $outputlangs->transnoentities("Payment"), 0, 'L', 0);
		$pdf->SetXY($tab3_posx - 11, $tab3_top);
		$pdf->MultiCell(20, 3, $outputlangs->transnoentities("Amount"), 0, 'L', 0);
		$pdf->SetXY($tab3_posx +11, $tab3_top);
		$pdf->MultiCell(20, 3, $outputlangs->transnoentities("Type"), 0, 'L', 0);
		$pdf->SetXY($tab3_posx + 24, $tab3_top);
		$pdf->MultiCell(20, 3, "Num Doc #", 0, 'L', 0);
		$pdf->SetXY($tab3_posx + 43, $tab3_top);
		$pdf->MultiCell(20, 3, "Transaction #", 0, 'L', 0);

		$pdf->line($tab3_posx-20, $tab3_top - 1 + $tab3_height, $tab3_posx + $tab3_width-17, $tab3_top - 1 + $tab3_height);

	
		//pdf_watermark($pdf, $outputlangs, $this->page_hauteur, $this->page_largeur, 'mm', "PAID");
		$pdf->SetTextColor(0, 0, 0);
	...
	protected function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		foreach ($object->linkedObjects as $objecttype => $objects)
		{
			if ($objecttype == 'shipping')
			{
				...
				{
				   	$ship = new Expedition($this->db);
				   	$ship->fetch('',$elementobject->ref);
					$pdf->SetFont('', '', $default_font_size-1 - $diffsizetitle);
					$pdf->writeHTMLCell(60, 4, $posx, $posy+3, $outputlangs->transnoentities("TrackingNumber")." : ".$ship->tracking_number, 0, 1, false, true, 'L');
					$pdf->writeHTMLCell(60, 4, $posx, $posy+13, $outputlangs->trans("LinkToTrackYourPackage")." : ".$ship->tracking_url, 0, 1, false, true, 'L');
				}
			...
			if ($objecttype == 'shipping')
			{
				...
				foreach ($objects as $x => $elementobject)
				{
				   	$lab = new Expedition($this->db);
				   	$lab->fetch('',$elementobject->ref);
					if ($lab->shipping_method_id )
							{
								// Get code using getLabelFromKey
								$code = $outputlangs->getLabelFromKey($this->db, $lab->shipping_method_id, 'c_shipment_mode', 'rowid', 'code');
								$label = '';
		...
		$pdf->SetXY($posx, $tab_top+35 + $tab_hl);
		$pdf->SetFont('', '', $default_font_size - 3);
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull)
			{
				...
				$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $total_ttc, 0, $outputlangs), $useborder, 'R', 1);
					...
					$pdf->SetXY($posx+120, $posy+30);
		$pdf->SetFont('', '', $default_font_size - 3);
		$pdf->MultiCell($col2x+23 - $col1x+16, $tab2_hl, "Except in the state of New Jersey - Country, State, County, Local Duties and Fees ( taxes ) are the responsibility of the receiving party", 'R',  1);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxqty - $this->posxup - 1, 2, $outputlangs->transnoentities("U.P."), '', 'C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxunit - $this->posxqty +2, 2, $outputlangs->transnoentities("Qty"), '', 'C');
		}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities("Paid Invoice Receipt");
		...
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell($w, 3, $outputlangs->transnoentities("Billing Date")." : ".dol_print_date($object->date, "day", false, $outputlangs)." / ", '', 'C');
		...
		if ($object->type != 2)
		{
			...
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 3, $outputlangs->transnoentities("Validity Ending Date")." : ".dol_print_date($object->date_lim_reglement, "day", false, $outputlangs, true), '', 'R');
		}
		...
		if ($showaddress)
		{
			...
			$pdf->RoundedRect($posx, $posy-4, $widthrecbox, $hautcadre+7, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));
			...
			$pdf->MultiCell(80, 4, "Order Processing Agent for Hielscher Ultrasonics GmbH", 0, 'L');
			$pdf->SetXY($posx + 2, $posy+3);
			$pdf->SetFont('', '', $default_font_size - 1.5);
			$pdf->MultiCell(80, 4, $carac_emetteur, 40, 'L');
			...
			$posy = !empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 40 : 42;
			$posy += $top_shift;
			$posx = $this->page_largeur - $this->marge_droite - $widthrecbox-16;
			if (!empty($conf->global->MAIN_INVERT_SENDER_RECIPIENT)) $posx = $this->marge_gauche;

			// Show recipient frame
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont('', '', $default_font_size - 2);
			$pdf->SetXY($posx + 1, $posy - 8);
			$pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("Bill to").":", 0, 'L');

			$pdf->SetXY($posx + 58, $posy - 8);
			$pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("Ship to").":", 0, 'L');
			// $pdf->Rect($posx, $posy, $widthrecbox, $hautcadre);
			$styles = array('width' => 0.25, 'cap' => 'butt', 'join' => 'miter', 'dash' => '3,3');		
			$pdf->RoundedRect($posx-2, $posy-4, $widthrecbox+19, $hautcadre+7, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));

			// Show recipient name
			$pdf->SetXY($posx + 1, $posy -2 );
			$pdf->SetFont('', 'B', $default_font_size-1);
			$pdf->MultiCell($widthrecbox-45, 2, $carac_client_name, 0, 'L');
			...
			$pdf->MultiCell($widthrecbox-60, 4, $carac_client, 0, 'L');
			...
			$pdf->MultiCell($widthrecbox, 4,$outputlangs->transnoentities("CustomerCode")." : ".$outputlangs->transnoentities($object->thirdparty->code_client), 0, 'L');
			...
			$posy = 0;
			// Show recipient name
			$pdf->SetXY($posx + 58, $posy +40 );
			$pdf->SetFont('', 'B', $default_font_size-1);
			$pdf->MultiCell($widthrecbox-45, 2, $carac_client_name, 0, 'L');

			// Show recipient name contact
			$pdf->SetXY($posx + 58, $posy + 44);
			$pdf->SetFont('', '', $default_font_size-1);
			$pdf->MultiCell($widthrecbox-40, 2, $carac_contact, 0, 'L');
