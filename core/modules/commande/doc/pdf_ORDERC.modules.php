 ...
 */
class pdf_ORDERC extends ModelePDFCommandes
{
	/**
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull)
			{
				...
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, "Total".(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transcountrynoentities("TotalTTC", $mysoc->country_code) : ''), $useborder, 'L', 1);
...
$pdf->MultiCell($widthrecbox, $hautcadre+5, "", 0,  'R', 1);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "PdfOrderTitle")
	{
		...
		$current_y = $pdf->getY();
		$posy = pdf_writeLinkedObjects($pdf, $object, $outputlangs, $posx, $posy-2, $w, 3, 'R', $default_font_size+1);
		...
		if ($showaddress)
		{
			...
			$pdf->RoundedRect($posx, $posy, $widthrecbox+90, $hautcadre, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));
			$pdf->SetTextColor(0, 0, 60);
