 ...
 */
class pdf_CONFORDER extends ModelePDFCommandes
{
    /**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->commande->dir_output)
		{
			...
			else
			{
				...
				$file = $dir."/Sale_Order_".$objectref."_".$this->emetteur->name."_".$object->thirdparty->name."_".$date_file."_".date("Hi",dol_now()).".pdf";
			}
			...
			if (file_exists($dir))
			{
				...
				if (!empty($conf->incoterm->enabled))
				{
					...
					if ($desc_incoterms)
					{
						...
						$pdf->SetFont('', '', $default_font_size - 1);
						$pdf->writeHTMLCell(190, 3, $this->posxdesc - 1, $tab_top -3, dol_htmlentitiesbr($desc_incoterms), 0, 1);
						global $user;
		...
		$arrayidcontact = $object->getIdContact('internal', 'SALESREPFOLL');
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxtva - $curX-11, 3, $curX+11, $curY, $hideref, $hidedesc);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						//$pdf->writeHTMLCell($this->posxcomment - $this->posxpiece - 0.8, 4, $curX+3, $curY, $i + 1, 0, 1); // Accountancy piece
						pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxtva - $curX-11, 4, $curX+11, $curY, $hideref, $hidedesc);
						$pageposafter = $pdf->getPage();
						$posyafter = $pdf->GetY();
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
					{
						...
						$pdf->MultiCell($this->posxup - $this->posxtva + 9, 3, $vat_rate, 0, 'C');
					//	$pdf->writeHTMLCell($this->posxcomment - $this->posxpiece - 0.8, 4, $curX+3, $curY, $i + 1, 0, 1); // Accountancy piece
					}
...
$pdf->MultiCell($widthrecbox, $hautcadre+5, "", 0,  'R', 1);
			...
			$pdf->SetXY($this->posxdesc + 10, $tab_top + 1);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey = "PdfOrderTitle")
	{
		...
		$pdf->MultiCell(100, 3, "Confirmation Order", '', 'R');
		...
		$current_y = $pdf->getY();
		$posy = pdf_writeLinkedObjects($pdf, $object, $outputlangs, $posx, $posy -1, 100, 3, 'R', $default_font_size+1);
		...
		if ($showaddress)
		{
			...
			$pdf->RoundedRect($posx, $posy, $widthrecbox+80, $hautcadre+5, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));
			$pdf->SetTextColor(0, 0, 60);
			...
			$posy = 38 + $top_shift;
			$posx = $this->page_largeur - $this->marge_droite - $widthrecbox-10;
			...
			//$pdf->Rect($posx, $posy, $widthrecbox, $hautcadre);
						$styles = array('width' => 0.25, 'cap' => 'butt', 'join' => 'miter', 'dash' => '3,3');		
			$pdf->RoundedRect($posx, $posy, $widthrecbox, $hautcadre+5, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));

			// Show recipient name
			$pdf->SetXY($posx + 2, $posy + 1);
			$pdf->SetFont('', 'B', $default_font_size-2);
			//$pdf->RoundedRect($this->marge_gauche, $tab_top+44 , $this->larg_util_cadre+75, $height_incoterms + 34, $this->Rounded_rect+3, '1111', null, $this->tblLineStyle);
			...
			// Show recipient information
			$pdf->SetFont('', '', $default_font_size - 2);
			$pdf->SetXY($posx + 55, $posy-1);
			$pdf->MultiCell($widthrecbox-35, 4, $carac_client, 0, 'L');
