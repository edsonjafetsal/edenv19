 ...
 */
class pdf_propalinv extends ModelePDFPropales
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->propal->multidir_output[$conf->entity])
		{
			...
			if ($object->specimen)
			{
				...
				$file = $dir."/PROFORMA_".$objectref."_".$this->emetteur->name."_".$object->thirdparty->name."_".$date_file."_".date("Hi",dol_now()).".pdf";
			}
			...
			if (file_exists($dir))
			{
				...
				if (!empty($conf->incoterm->enabled))
				{
					...
					if ($desc_incoterms)
					{
						...
						$pdf->SetFont('', '', $default_font_size - 1);
						$pdf->writeHTMLCell(190, 3, $this->posxdesc - 1, $tab_top - 6, dol_htmlentitiesbr($desc_incoterms), 0, 1);
						global $user;
		...
		$arrayidcontact = $object->getIdContact('internal', 'SALESREPFOLL');
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					//$pdf->writeHTMLCell($this->posxcomment - $this->posxpiece - 0.8, 4, $curX+4, $curY, $i + 1, 0, 1); // Accountancy piece
					pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxpicture - $curX-11, 3, $curX+11 , $curY, $hideref, $hidedesc);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						$pdf->setPageOrientation('', 1, $heightforfooter+8); // The only function to edit the bottom margin of current page to set it.
						//$pdf->writeHTMLCell($this->posxcomment - $this->posxpiece - 0.8, 4, $curX+4, $curY, $i + 1, 0, 1); // Accountancy piece
						pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxpicture - $curX-11, 3, $curX+11, $curY, $hideref, $hidedesc);
					...
					$pdf->MultiCell($this->posxup - $this->posxtva - 6, 23,  $i + 1, 0, 'L');
				...
				if ($pagenb == 1)
				{
					...
					$this->_tableau($pdf, $tab_top_newpage-1, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter, 0, $outputlangs, 1, 0, $object->multicurrency_code);
					$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter + 1;
				}
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop))
		{
			$pdf->line($this->marge_gauche, $tab_top + 5, $this->page_largeur - $this->marge_droite, $tab_top + 5); // line takes a position y in 2nd parameter and 4th parameter
						$pdf->SetXY($posx+5, $tab_top);
$pdf->SetFillColor(209, 228, 247);
$pdf->MultiCell($widthrecbox, $hautcadre+5, "", 0,  'R', 1);

			$pdf->SetXY($this->posxdesc+3 , $tab_top + 1);
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxqty - $this->posxup - 1, 2, "U.P.", '', 'C');
		}
		...
		if (empty($hidetop))
		{
			...
			//$pdf->MultiCell($this->posxqty - $this->posxup - 1, 2, $outputlangs->transnoentities("Line"), '', 'C');
		}
	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->MultiCell(100, 4, "Proforma Invoice", '', 'R');
		...
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Date")." : ".dol_print_date($object->date, "day", false, $outputlangs, true)." / ", '', 'C');
		$posy += 0;
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		...
		$current_y = $pdf->getY();
		$posy = pdf_writeLinkedObjects($pdf, $object, $outputlangs, $posx, $posy-1, 100, 3, 'R', $default_font_size);
		...
		$pdf->SetFont('', '', $default_font_size-2  );
        $pdf->MultiCell($largcol,5, "Editor"." : ".$userstatic->firstname.' '.$userstatic->lastname,0,'R',1);
		...
		if ($showaddress)
		{
			...
			$pdf->RoundedRect($posx, $posy, $widthrecbox+80, $hautcadre+4, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));
			...
			//$pdf->MultiCell(80, 4, $outputlangs->convToOutputCharset($this->emetteur->country), 0, 'L');
		...
		// Recipient name
			...
			//$pdf->RoundedRect($this->marge_gauche, $tab_top+42 , $this->larg_util_cadre+90, $height_incoterms + 34, $this->Rounded_rect+3, '1111', null, $this->tblLineStyle);
			//$pdf->RoundedRect($this->marge_gauche+94, $tab_top+42 , $this->larg_util_cadre+110, $height_incoterms + 39, $this->Rounded_rect+3, '1111', null, $this->tblLineStyle);
			if (!empty($conf->global->MAIN_INVERT_SENDER_RECIPIENT)) $posx = $this->marge_gauche;

			// Show recipient frame
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont('', '', $default_font_size - 2);
			$pdf->SetXY($posx - 19, $posy - 5);
			$pdf->MultiCell($widthrecbox, 5, "Bill To:                                                                   Ship to".":", 0, 'L');
			//$pdf->Rect($posx, $posy, $widthrecbox, $hautcadre);
									$styles = array('width' => 0.25, 'cap' => 'butt', 'join' => 'miter', 'dash' => '3,3');		
			$pdf->RoundedRect($posx-22, $posy, $widthrecbox+22, $hautcadre+4, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));


			// Show recipient name
			$pdf->SetXY($posx -20, $posy + 1);
			$pdf->SetFont('', 'B', $default_font_size+1);
			$pdf->MultiCell($widthrecbox-50, 4, $carac_client_name, 0, 'L');
			...
			$carac_client = pdf_build_address($outputlangs, $this->emetteur, $object->contact, ($usecontact ? $object->contact : ''), $usecontact, 'target', $object);
						// Show recipient name
			$pdf->SetXY($posx + 39, $posy-8);
			$pdf->SetFont('', 'B', $default_font_size);
			$pdf->MultiCell($widthrecbox-35, 2, $carac_client_name, 0, 'L');

			$posy = $pdf->getY();

			// Show recipient information
			$pdf->SetFont('', '', $default_font_size - 2);
			$pdf->SetXY($posx + 39, $posy+1);
	...
	protected function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->SetXY($posx, $tab_top+13 + $tab_hl);
