 ...
 */
class pdf_PROFORMA extends ModelePDFPropales
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$arrayidcontact = $object->getIdContact('internal', 'SALESREPFOLL');
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxpicture - $curX-12, 3, $curX +11 , $curY, $hideref, $hidedesc);
					...
					//	$pdf->writeHTMLCell($this->posxcomment - $this->posxpiece - 0.8, 4, $curX+3, $curY, $i + 1, 0, 1); // Accountancy piece
						pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxpicture - $curX-10, 3, $curX+11, $curY, $hideref, $hidedesc);
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
					{
						...
						$pdf->MultiCell($this->posxup - $this->posxtva + 8, 3, $vat_rate, 0, 'C');
						//$pdf->writeHTMLCell($this->posxcomment - $this->posxpiece - 0.8, 4, $curX+3, $curY, $i + 1, 0, 1); // Accountancy piece
					}
					...
					$pdf->MultiCell($this->posxqty - $this->posxup - 6, 3, $up_excl_tax, 0, 'R', 0);
				...
				if ($pagenb == 1)
				{
					...
					$this->_tableau($pdf, $tab_top_newpage-2, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter, 0, $outputlangs, 1, 0, $object->multicurrency_code);
					$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter + 1;
				}
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop))
		{
			$pdf->line($this->marge_gauche, $tab_top + 5, $this->page_largeur+6 - $this->marge_droite, $tab_top + 5); // line takes a position y in 2nd parameter and 4th parameter
...
$pdf->MultiCell($widthrecbox, $hautcadre+5, "", 0,  'R', 1);
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxqty - $this->posxup - 1, 2, '', 'C');
		}
	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if (!empty($conf->global->DOC_SHOW_FIRST_SALES_REP))
		{
			...
			if (count($arrayidcontact) > 0)
			{
				...
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(100, 3, $langs->trans("SalesRepresentative")." : ".$usertmp->getFullName($langs),$usertmp->email,  '', 'R');
			}
		}
						global $conf;
		$default_font_size = pdf_getPDFFontSize($outputlangs);
		...
		if ($showaddress)
		{
			...
			$pdf->RoundedRect($posx, $posy, $widthrecbox+90, $hautcadre+3, 3, '1111', null, $styles);
			$pdf->SetLineStyle(array('dash'=>0));
			...
			$posx = $this->page_largeur - $this->marge_droite - $widthrecbox;
					//	$pdf->RoundedRect($this->marge_gauche, $tab_top+44 , $this->larg_util_cadre+90, $height_incoterms + 34, $this->Rounded_rect+3, '1111', null, $this->tblLineStyle);
			//$pdf->RoundedRect($this->marge_gauche+105, $tab_top+44 , $this->larg_util_cadre+90, $height_incoterms + 39, $this->Rounded_rect+3, '1111', null, $this->tblLineStyle);
